...
\section{Limit of Extreme Eigenvalues}
...
\begin{equation} \label{eq:limit-of-extreme-eigenvalues-of-sample-covariance-matrices}
    (1-\sqrt{\rho})^{2}=\lim_{n\rightarrow\infty}\lambda_{\min}\left(\widehat{\Sigma}_{n}\right)\leq\lim_{n\rightarrow\infty}\lambda_{\max}\left(\widehat{\Sigma}_{n}\right)=(1+\sqrt{\rho})^{2}\quad\text{ a.s.}
\end{equation}
where the first inequality is considered only in the case where $m_{n}\geq n$.

\begin{remark}
    The convergence of the smallest eigenvalue in the left hand side of (\ref{eq:limit-of-extreme-eigenvalues-of-sample-covariance-matrices}) holds if $\left(X_{n,k}\right)_{n\geq 1,1\leq k\leq n}$ are i.i.d. with finite second moment.
\end{remark}