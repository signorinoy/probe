...
\begin{property}
    ...
    \begin{enumerate}
        \item complex analytic on $\mathbb{C}\backslash\operatorname{supp}(\mu)$.
        \item
    \end{enumerate}
\end{property}
...
\begin{theorem}[Inverse Stieltjes Transform]
    ...
    \begin{equation}
        \mu\left([a,b]\right)=\frac{1}{\pi}\lim_{y\downarrow 0}\int_{a}^{b}\operatorname{Im}\left[m_{\mu}(x+\mathrm{i}y)\right]\,\mathrm{d}x
    \end{equation}
    Specially, if $\mu$ has a density $f$ at $x$, then
    \begin{equation}
        f(x)=\frac{1}{\pi}\lim_{y\downarrow 0}\operatorname{Im}\left[m_{\mu}(x+\mathrm{i}y)\right]
    \end{equation}
    And, if $\mu$ has an isolated mass at $x$, then
    \begin{equation}
        \mu(\{x\})=\lim_{y \downarrow 0}-\mathrm{i}y m_{\mu}(x+\mathrm{i}y)
    \end{equation}
\end{theorem}

\begin{proof}
    \begin{equation*}
        \frac{1}{\pi}\int_{a}^{b}\operatorname{Im}\left[m_{\mu}(x+\mathrm{i}y)\right]\,\mathrm{d}x=\frac{1}{\pi}\int_{a}^{b}\left[\int\frac{y}{(t-x)^{2}+y^{2}}\mu(\mathrm{d}t)\right]\,\mathrm{d}x
    \end{equation*}
    By Fubini's theorem,
    \begin{equation*}
        \begin{aligned}
    ...
    \begin{equation*}
        \frac{1}{\pi}\lim_{y\downarrow 0}\int_{a}^{b}\operatorname{Im}\left[m_{\mu}(x+\mathrm{i}y)\right]\,\mathrm{d}x=\frac{1}{\pi}\int\lim_{y\downarrow 0}\left[\arctan\left(\frac{b-t}{y}\right)-\arctan\left(\frac{a-t}{y}\right)\right]\mu(\mathrm{d}t)
    \end{equation*}
    ...
    \begin{equation*}
        \lim_{y\downarrow 0}-\mathrm{i}ym_{\mu}(x+\mathrm{i}y)=-\lim_{y\downarrow 0}\int\frac{\mathrm{i}y(t-x)\mu(\mathrm{d}t)}{(t-x)^{2}+y^{2}}+\lim_{y\downarrow 0}\int\frac{y^{2}\mu(\mathrm{d}t)}{(t-x)^{2}+y^{2}}=a
    \end{equation*}
\end{proof}
...
\section{Extremal Eigenvalues of}
